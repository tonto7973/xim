    ...
    public class XimSbSimulatorSmokeTests
    {
        ...
        public async Task TestAzureServiceBusQueueReceive()
        {
            ...
            using (var simulation = Simulation.Create())
            {
                ...
                var bus = simulation.AddServiceBus()
                    .SetCertificate(testCertificate)
                    .AddQueue(queueName)
                    .Build();

                await bus.StartAsync();
                var busQueue = bus.Queues[queueName];
                ...
                try
                {
                    ...
                    var deliveryTasks = deliveries.Select(d => d.WaitAsync(TimeSpan.FromSeconds(5)));
        ...
        public async Task TestAzureServiceBusQueueSend()
        {
            ...
            using (var simulation = Simulation.Create())
            {
                ...
                var deliveries = bus.Queues[queueName].Deliveries;
