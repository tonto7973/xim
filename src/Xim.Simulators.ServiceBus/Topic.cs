    ...
    public class Topic
    {
        ...
        public Topic(string name, params Subscription[] subscriptions)
        {
            ...
            if (!RxValidName.IsMatch(name))
                throw new ArgumentException(SR.Format(SR.SbEntityNameNotValid, RxValidName), nameof(name));
            if (subscriptions?.Any(s => s == null) == true)
                throw new ArgumentException(SR.Format(SR.SbTopicSubscriptionNull), nameof(subscriptions));
            if (subscriptions?
                .Select(s => s.Name)
                .GroupBy(n => n, StringComparer.InvariantCultureIgnoreCase)
                .FirstOrDefault(g => g.Count() > 1) is IGrouping<string, string> duplicate)
            {
                throw new ArgumentException(SR.Format(SR.SbTopicSubscriptionNotUnique, duplicate.Key), nameof(subscriptions));
            }
